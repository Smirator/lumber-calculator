...
const humidity = 500; //влажность имитации бруса
...
//параметры для третьего калькулятора
valueTransfer(
    document.getElementById('select-options-help-material'),
    document.getElementById('height-material-square'),
    document.getElementById('width-material-square'),
    document.getElementById('length-material-square'),
    calculateAmount
); 
...
function calculateOptions() {
    ...
    // проверим, все ли инпуты содержат значения, подсветим пустые
    ...
    if (checkVarValue == true) { //условие проверки, для того, чтобы не выводить значения NAN
        ...
        if (measure.value === 'cubes') {
            document.getElementById('result').innerHTML = Math.ceil(totalItems) + '  шт. имитации бруса в указанном объеме (с округлением до целого числа в большую сторону)'
        } else {
            document.getElementById('result').innerHTML = countm3(totalItems).toFixed(3) + ' м³ объем данного количества имитации бруса';
        }
        document.getElementById('weight-result').innerHTML = (weight(totalItems)).toFixed(2) + ' кг составляет примерный вес данного количества имитации бруса';
    } else {
...
function calculateAmountSide() {
    ...
    if (checkVarValue == true) { //Проверяем и выводим значения
        ...
        if (sideArea <= 0 || amountМaterial <= 0) {
            ...
            document.getElementById('number-pieces-result-side').innerHTML = Math.ceil(payment()) + ' нужное количество шт. имитации бруса (с округлением до целого числа в большую сторону)';
            document.getElementById('volume-material-side').innerHTML = amountМaterial.toFixed(2) + ' м³ необходимый объем имитации бруса';
            document.getElementById('weight-result-material-side').innerHTML = weight().toFixed(2) + ' кг, примерный вес данного количества имитации бруса';
...
function calculateAmount() {
    ...
    let heightValue = Number(document.getElementById('height-material-square').value);//высота, явно указываем, что тип значения number
    let widthValue = Number(document.getElementById('width-material-square').value);//ширина
    let lengthValue = Number(document.getElementById('length-material-square').value); //длина

    let widthRoomValue = Number(document.getElementById('width-room').value); //ширина помещения
    let lengthRoomValue = Number(document.getElementById('length-room').value); //ширина помещения
    let heightRoomValue = Number(document.getElementById('height-room').value); //высота помещения

    let arrSumDeletedValue = []; //массив со сложенными значениями, которые вычитаем из площади

    checkInputTypeValue(importantSquare);


    // Передаем в массив значения для вычитаемой площади
    for (let j = 0, l = arrDeletedValueSquare.length; j < l; j++) {
        let sumDeletedValue = (arrDeletedValueSquare[j].heightAperture * arrDeletedValueSquare[j].widthAperture / 1000000) * arrDeletedValueSquare[j].amountAperture;
        arrSumDeletedValue.push(sumDeletedValue);
    }

    //Складываем все элементы массива в одно число для вычитания их из площади.
    if (!arrSumDeletedValue.length == 0) {
        area = arrSumDeletedValue.reduce((total, amount) => total + amount);
    } else {
        area = false;
    }
    ...
    let homeArea = (widthRoomValue * 2) + (lengthRoomValue * 2); //периметр дома
    let fullHomeArea = (homeArea * heightRoomValue) - area;//площадь
    let amountМaterial = (fullHomeArea * heightValue) / 1000 // Расчет количества материала, в кубах
    ...
    let checkVarValue = [widthValue, heightValue, lengthValue, widthRoomValue, lengthRoomValue, heightRoomValue,].every(checkTypeValue);//в переменную передаем true или false. Если false, то дальше не пускает


    if (checkVarValue == true) { //Проверяем и выводим значения
        if (fullHomeArea <= 0 || amountМaterial <= 0) {
            animateResult([document.getElementById('room-area-result'), document.getElementById('number-pieces-result'), document.getElementById('weight-result-material'), document.getElementById('volume-material'),]); //анимация
            changeClass([document.getElementById('initial-msg-calc-room')], "add", "d-n");
            document.getElementById('room-area-result').innerHTML = fullHomeArea.toFixed(2) + ' м² площадь стен';
            document.getElementById('number-pieces-result').innerHTML = Math.ceil(payment()) + ' нужное количество шт. имитации бруса (с округлением до целого числа в большую сторону)';
            document.getElementById('volume-material').innerHTML = amountМaterial.toFixed(2) + ' м³ необходимый объем имитации бруса';
            document.getElementById('weight-result-material').innerHTML = weight().toFixed(2) + ' кг примерный вес данного количества имитации бруса';
            document.getElementById('room-area-result').innerHTML = 'Вычитаемая площадь больше объема введенного материала, проверьте введенные значения';
        } else {
            animateResult([document.getElementById('room-area-result'), document.getElementById('number-pieces-result'), document.getElementById('volume-material'), document.getElementById('weight-result-material'),]);
            changeClass([document.getElementById('initial-msg-calc-room')], "add", "d-n");
            document.getElementById('room-area-result').innerHTML = fullHomeArea.toFixed(2) + ' м² площадь стен';
            document.getElementById('number-pieces-result').innerHTML = Math.ceil(payment()) + ' нужное количество шт. имитации бруса (с округлением до целого числа в большую сторону)';
            document.getElementById('volume-material').innerHTML = amountМaterial.toFixed(2) + ' м³ необходимый объем имитации бруса';
        ...
        changeClass([document.getElementById('initial-msg-calc-room')], "remove", "d-n");
        changeClass([
            document.getElementById('room-area-result'),
            document.getElementById('number-pieces-result'),
            document.getElementById('volume-material'),
            document.getElementById('weight-result-material'),
            document.getElementById('room-area-result')
        ], "add", "d-n");
    }
}
...
excludedSize( //Передача параметров вычитаемой площади в расчете на 4 стороны.
    /*Кнопка активации функции:*/
    document.getElementById('btn-deleted-values-plus-square'), //btnDeletePlus

    /*Дефолтный текст и блок*/
    document.getElementById('text-deleted-value-square'),//textDeletedValue
    document.getElementById('calculator-deleted-values-block-square'),//deletedValueBlock
    document.getElementById('btn-deleted-values-false-square'),//btnDeleteFalse

    /*Параметры: ширина, длина, количество:*/
    document.getElementById('width-aperture-square'), //widthExcluded,
    document.getElementById('height-aperture-square'), //heightExcluded
    document.getElementById('amount-aperture-square'), //amountExcluded

    /*Коллекция для проверки:*/
    document.querySelectorAll('.deleted-value-square'), //listEl

    /*Список, в который передаем значения:*/
    document.getElementById('deleted-value-el-ul-list-square'),//ulDeletedValue

    /*Массивы, исключаемой площади*/
    arrDeletedValueSquare, comparableArrDeletedValueSquare, //arrDeletedValue, comparableArrDeletedValue 

    /*Уведомление о повторе размеров*/
    document.getElementById('notification-square'), //notificationResult

    /*Вызов функции на пересчет:*/
    calculateAmount//functionCalcName
);



disappearExcludedSize(//исчезание блока для калькулятора, рассчитывающего на 4 стороны
    document.getElementById('btn-deleted-values-false-square'),//btnDeleteFalse
    document.getElementById('calculator-deleted-values-block-square'),//deletedValueBlock
    document.getElementById('text-deleted-value-square'),//textDeletedValue
    document.getElementById('deleted-value-el-ul-list-square'),//ulDeletedValue
    arrDeletedValueSquare, //arrDeletedValue
    calculateAmount,//functionCalcName
    /*Параметры: ширина, длина, количество:*/
    document.getElementById('width-aperture-square'), //widthExcluded
    document.getElementById('height-aperture-square'), //heightExcluded
...
// если пользователь ввел значения, запускаем расчет на помещение
trackingInput([
    document.getElementById('width-room'),
    document.getElementById('length-room'), 
    document.getElementById('height-room'), 
    document.getElementById('height-material-square'),
    document.getElementById('width-material-square'),
    document.getElementById('length-material-square'),
    ], calculateAmount
